...
class ProductDAL extends Database{
	...
	public function getTopHot($number){
		$query = "SELECT ID,ProductName,IDCate,Image,Price,Quantity,NewQuantity FROM product WHERE STATUS = true ORDER BY Count DESC LIMIT $number";
		$result = mysqli_query($this->connectionString,$query);
		$array = array();
		while ($rows = mysqli_fetch_assoc($result)) {
			$array[] = $rows;
	...
	public function getTopNew($number){
		$query = "SELECT ID,ProductName,IDCate,Image,Price,Quantity,NewQuantity FROM product WHERE STATUS = true ORDER BY CreatedDay DESC LIMIT $number";
		$result = mysqli_query($this->connectionString,$query);
		$array = array();
		while ($rows = mysqli_fetch_assoc($result)) {
			$array[] = $rows;
	...
	public function getTopView($number){
		$query = "SELECT ID,ProductName,IDCate,Image,Price,View,Quantity,NewQuantity FROM product WHERE STATUS = true ORDER BY View DESC LIMIT $number";
		$result = mysqli_query($this->connectionString,$query);
		$array = array();
		while ($rows = mysqli_fetch_assoc($result)) {
			$array[] = $rows;
	...
	public function getRelatedProduct($productCateID){
		$query = "SELECT ID,ProductName,Image,Price,Quantity,NewQuantity FROM product WHERE IDCate = $productCateID";
		$result = mysqli_query($this->connectionString,$query);
		$array = array();
		while ($rows = mysqli_fetch_assoc($result)) {
			$array[] = $rows;
